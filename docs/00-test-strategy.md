...
| **2. Analysis** | Divide each difficulty into as many parts as needed | "Is the product good?" is untestable. Divided into nine scopes, each answers exactly **one** question (see the scope map in the README). A test that answers two questions is two tests badly merged. |
...
Every scope document (01–09) is structured as exactly these three sections. Test cases in the [catalog](10-test-case-catalog.md) follow the same split: preconditions → steps → expected result. So does [defect reporting](11-defect-reporting.md): isolate → write → verify. One mental model, applied recursively.
...
| --- | --- | --- | --- |
| R1 | An order is priced wrong (discount, shipping, rounding) | Critical | Unit (exhaustive boundaries), API, Regression |
| R2 | A user reads another user's data | Critical | Security (IDOR, session isolation) |
...
Ordering precept applied to the pipeline — three gates, each one cheaper and more precise than the next, each able to veto everything after it:

| Gate | Suites | Typical duration | Vetoes |
| --- | --- | --- | --- |
| 1 | typecheck, unit, contract | seconds | everything |
| 2 | smoke → api, security, a11y, regression, e2e | ~1 min | gate 3 |
| 3 | load baseline (k6) | ~1 min | release |
...
Docs 01–09 follow the pyramid order — simplest scope first. Each is self-contained, ~5 minutes, and ends with field notes: the judgment calls a checklist cannot give you.
